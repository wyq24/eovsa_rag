# Schedule Commands - EOVSA Wiki

**Source:** https://www.ovsa.njit.edu/wiki/index.php/Schedule_Commands
**Scraped:** 2025-08-05 09:12:49
...
_**atten[pol,tuning_index]=base_atten[pol,tuning_index]+regime*2db**_
...
_**HPOL_ATTENUATION=HPOL_BASE+FEMATTN.txt(level)**_

_**VPOL_ATTENUATION=VPOL_BASE+FEMTTTN.txt(level)**_
...
Activates (1) or deactivates (0) the front end automatic gain control loop, which adjust the FEMATTN level when both of the HPOL and VPOL voltages, averaged over the given number of consecutive _**samples**_ , cross the _**low**_ and _**high**_ voltage limits. Missing parameter values are uploaded from the [FEM AGC] section of the local “c:\ni-rt\startup\crio.ini” file. All parameters provided when an AGC command is issued are saved in the same file for further use. 

NOTE: At startup, the AGC loop is activated or not according with the latest _**active**_ value written in the initialization file. 
...
NOTE: At startup, the WINDSCRAM loop is activated or not according with the latest _**active**_ written in the initialization file. 
...
Retrieved from "[http://ovsa.njit.edu//wiki/index.php?title=Schedule_Commands&oldid=469](http://ovsa.njit.edu//wiki/index.php?title=Schedule_Commands&oldid=469)"
