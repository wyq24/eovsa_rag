...
def sp_bsize(x,y):
    ...
    for ant in range(nant):
        j = ant//nrow
        i = ant % nrow
        ax[i,j].set_title('Ant '+str(ant+1)+' [blue=RA, red=Dec]')
        if ant % ncol == 0:
            ax[ant//ncol,0].set_ylabel('Beam FWHM [deg]')
        if ant >= ncol:
            ax[1,ant - ncol].set_xlabel('Frequency [GHz]')
        ax[i,j].set_ylim(0.5,20)
        ax[i,j].set_xlim(1,20)
        ax[i,j].set_yscale('log')
        ax[i,j].set_xscale('log')
        ax[i,j].plot(x['fghz'][fgood],x['raparms'][2,fgood,ant]*2*np.sqrt(np.log(2))/10000,'b.')
        ax[i,j].plot(fout,aout,color='black')
        ax[i,j].plot(x['fghz'][fgood],x['decparms'][2,fgood,ant]*2*np.sqrt(np.log(2))/10000,'r.')
    plt.draw()
    f, ax = plt.subplots(nrow, ncol, sharex='col', sharey='row')
    f.set_size_inches(2*nant,7,forward=True)
    t1 = Time(x['ut_mjd'][0],format='mjd')
    ...
    for ant in range(nant):
        ...
        ax[i,j].set_yscale('log')
        ax[i,j].set_xscale('log')
        ax[i,j].plot(y['fghz'][fgood],y['raparms'][2,fgood,ant]*2*np.sqrt(np.log(2))/10000,'b.')
        ax[i,j].plot(fout,aout,color='black')
        ax[i,j].plot(y['fghz'][fgood],y['decparms'][2,fgood,ant]*2*np.sqrt(np.log(2))/10000,'r.')
    plt.draw()
...
def sp_offsets(x,y,save_plot=False):
    ...
    for ant in range(nant):
        ...
        ax[j,i].set_title('Ant '+str(ant+1)+' [blue=RA, red=Dec]')
        if ant >= nant-2:
            ax[nant//ncol-1,ant % ncol].set_xlabel('Frequency [GHz]')
        if ant % ncol == 0:
            ax[j,i].set_ylabel('Offset [deg]')
        ramed = np.median(x['raparms'][1,fgood,ant])
        ax[j,i].plot(x['fghz'][fgood],x['raparms'][1,fgood,ant]/10000,'b.')
        ax[j,i].plot([0,18],np.array([1,1])*ramed/10000,'b')
        decmed = np.median(x['decparms'][1,fgood,ant])
        ax[j,i].plot(x['fghz'][fgood],x['decparms'][1,fgood,ant]/10000,'r.')
        ax[j,i].plot([0,18],np.array([1,1])*decmed/10000,'r')
        if ant in oldant:
            ax[j,i].text(0.05,0.80,'RA = {:6.3f}'.format(ramed/10000.),color='red',transform=ax[ant//2,ant % 2].transAxes)
            ax[j,i].text(0.05,0.65,'Dec = {:6.3f}'.format(decmed/10000.),color='red',transform=ax[ant//2,ant % 2].transAxes)
        else:
            ax[j,i].text(0.05,0.80,'RA = {:6.3f}'.format(ramed/10000.),color='gray',transform=ax[ant//2,ant % 2].transAxes)
            ax[j,i].text(0.05,0.65,'Dec = {:6.3f}'.format(decmed/10000.),color='gray',transform=ax[ant//2,ant % 2].transAxes)
        xel, el = solpnt.dradec2dazel(x['ra0'],x['dec0'],t1,ramed*user2rad/cosdec,decmed*user2rad)
        if ant in oldant:
            ax[j,i].text(0.65,0.80,'XEL = {:6.3f}'.format(xel*180./np.pi),color='gray',transform=ax[ant//2,ant % 2].transAxes)
            ax[j,i].text(0.65,0.65,'EL = {:6.3f}'.format(el*180./np.pi),color='gray',transform=ax[ant//2,ant % 2].transAxes)
        else:
            ax[j,i].text(0.65,0.80,'XEL = {:6.3f}'.format(xel*180./np.pi),color='red',transform=ax[ant//2,ant % 2].transAxes)
            ax[j,i].text(0.65,0.65,'EL = {:6.3f}'.format(el*180./np.pi),color='red',transform=ax[ant//2,ant % 2].transAxes)
        if ant in oldant:
    ...
    for ant in range(nant):
        ...
        ax[j,i].plot(y['fghz'][fgood],y['raparms'][1,fgood,ant]/10000,'b.')
        ax[j,i].plot([0,18],np.array([1,1])*ramed/10000,'b')
        decmed = np.median(y['decparms'][1,fgood,ant])
        ax[j,i].plot(y['fghz'][fgood],y['decparms'][1,fgood,ant]/10000,'r.')
        ax[j,i].plot([0,18],np.array([1,1])*decmed/10000,'r')
        if ant in oldant:
            ax[j,i].text(0.05,0.80,'RA = {:6.3f}'.format(ramed/10000.),color='red',transform=ax[ant//2,ant % 2].transAxes)
            ax[j,i].text(0.05,0.65,'Dec = {:6.3f}'.format(decmed/10000.),color='red',transform=ax[ant//2,ant % 2].transAxes)
