...
#            self.fig_info[-1][1].set_position([bbox[0]-0.05,bbox[1],bbox[2]-bbox[0],bbox[3]-bbox[1]+0.1])
        for i in range(3):
            ...
            canvas.draw()
            canvas.get_tk_widget().pack(side=Tk.TOP, fill=Tk.BOTH, expand=1)
            toolbar = NavigationToolbar2Tk(canvas, fant[i])
            toolbar.update()
            ...
            if line[-1] == 'R':
                ...
                for i in range(nsolant):
                    for j in range(2):
                        ...
                        if i != 0:
                            ticks_loc = ax1[j,i].get_yticks().tolist()
                            ax1[j,i].yaxis.set_major_locator(FixedLocator(ticks_loc))
                            ax2[j,i].yaxis.set_major_locator(FixedLocator(ticks_loc))
                ...
                ax = self.ab_fig_info[1]
                im = ax.pcolormesh(np.arange(16),np.arange(self.maxnbd+1),np.zeros((self.maxnbd,13)))
...
def phase_diff(phacal, refcal):
    ...
    slopes = np.zeros((nant,2),np.float64)
    offsets = np.zeros((nant,2),np.float64)
    flag = np.zeros((nant,2),np.float64)
...
def refcal_anal(out):
    ...
    if 'tflags' in list(out.keys()):
        ...
        out.update({'tflags':np.zeros((nant,maxnbd,2,2),np.float64)})
    nt = len(times)
